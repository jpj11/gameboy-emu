...
short DecodeExecute(BYTE opcode, FILE *output)
{
    ...
    switch(opcode)
    {
        ...
        case 0xe2: cycles = LoadByte(&mainMemory[regBC.lo + REG_P1], memory, regAF.hi, reg);
                   fprintf(output, OPRR, "LD", "(C)", "A");
                   break;
        case 0xf2: cycles = LoadByte(&regAF.hi, reg, mainMemory[regBC.lo + REG_P1], memory);
                   fprintf(output, OPRR, "LD", "A", "(C)");
                   break;
...
short DecodeExecuteCB(BYTE opcode, FILE *output)
{
    ...
    switch (opcode)
    {
        ...
        // Test bit 0
        case 0x40: cycles = Bit(0, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "0", "B");
                   break;
        case 0x41: cycles = Bit(0, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "0", "C");
                   break;
        case 0x42: cycles = Bit(0, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "0", "D");
                   break;
        case 0x43: cycles = Bit(0, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "0", "E");
                   break;
        case 0x44: cycles = Bit(0, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "0", "H");
                   break;
        case 0x45: cycles = Bit(0, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "0", "L");
                   break;
        case 0x46: cycles = Bit(0, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "0", "(HL)");
                   break;
        case 0x47: cycles = Bit(0, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "0", "A");
                   break;

        // Test bit 1
        case 0x48: cycles = Bit(1, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "1", "B");
                   break;
        case 0x49: cycles = Bit(1, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "1", "C");
                   break;
        case 0x4a: cycles = Bit(1, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "1", "D");
                   break;
        case 0x4b: cycles = Bit(1, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "1", "E");
                   break;
        case 0x4c: cycles = Bit(1, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "1", "H");
                   break;
        case 0x4d: cycles = Bit(1, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "1", "L");
                   break;
        case 0x4e: cycles = Bit(1, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "1", "(HL)");
                   break;
        case 0x4f: cycles = Bit(1, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "1", "A");
                   break;

        // Test bit 2
        case 0x50: cycles = Bit(2, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "2", "B");
                   break;
        case 0x51: cycles = Bit(2, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "2", "C");
                   break;
        case 0x52: cycles = Bit(2, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "2", "D");
                   break;
        case 0x53: cycles = Bit(2, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "2", "E");
                   break;
        case 0x54: cycles = Bit(2, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "2", "H");
                   break;
        case 0x55: cycles = Bit(2, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "2", "L");
                   break;
        case 0x56: cycles = Bit(2, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "2", "(HL)");
                   break;
        case 0x57: cycles = Bit(2, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "2", "A");
                   break;

        // Test bit 3
        case 0x58: cycles = Bit(3, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "3", "B");
                   break;
        case 0x59: cycles = Bit(3, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "3", "C");
                   break;
        case 0x5a: cycles = Bit(3, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "3", "D");
                   break;
        case 0x5b: cycles = Bit(3, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "3", "E");
                   break;
        case 0x5c: cycles = Bit(3, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "3", "H");
                   break;
        case 0x5d: cycles = Bit(3, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "3", "L");
                   break;
        case 0x5e: cycles = Bit(3, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "3", "(HL)");
                   break;
        case 0x5f: cycles = Bit(3, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "3", "A");
                   break;

        // Test bit 4
        case 0x60: cycles = Bit(4, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "4", "B");
                   break;
        case 0x61: cycles = Bit(4, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "4", "C");
                   break;
        case 0x62: cycles = Bit(4, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "4", "D");
                   break;
        case 0x63: cycles = Bit(4, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "4", "E");
                   break;
        case 0x64: cycles = Bit(4, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "4", "H");
                   break;
        case 0x65: cycles = Bit(4, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "4", "L");
                   break;
        case 0x66: cycles = Bit(4, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "4", "(HL)");
                   break;
        case 0x67: cycles = Bit(4, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "4", "A");
                   break;                    

        // Test bit 5
        case 0x68: cycles = Bit(5, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "5", "B");
                   break;
        case 0x69: cycles = Bit(5, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "5", "C");
                   break;
        case 0x6a: cycles = Bit(5, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "5", "D");
                   break;
        case 0x6b: cycles = Bit(5, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "5", "E");
                   break;
        case 0x6c: cycles = Bit(5, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "5", "H");
                   break;
        case 0x6d: cycles = Bit(5, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "5", "L");
                   break;
        case 0x6e: cycles = Bit(5, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "5", "(HL)");
                   break;
        case 0x6f: cycles = Bit(5, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "5", "A");
                   break;        
                   
        // Test bit 6
        case 0x70: cycles = Bit(6, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "6", "B");
                   break;
        case 0x71: cycles = Bit(6, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "6", "C");
                   break;
        case 0x72: cycles = Bit(6, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "6", "D");
                   break;
        case 0x73: cycles = Bit(6, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "6", "E");
                   break;
        case 0x74: cycles = Bit(6, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "6", "H");
                   break;
        case 0x75: cycles = Bit(6, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "6", "L");
                   break;
        case 0x76: cycles = Bit(6, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "6", "(HL)");
                   break;
        case 0x77: cycles = Bit(6, &regAF.hi);
				   fprintf(output, OPRR, "BIT", "6", "A");
                   break;                    

        // Test bit 7
        case 0x78: cycles = Bit(7, &regBC.hi);
				   fprintf(output, OPRR, "BIT", "7", "B");
                   break;
        case 0x79: cycles = Bit(7, &regBC.lo);
				   fprintf(output, OPRR, "BIT", "7", "C");
                   break;
        case 0x7a: cycles = Bit(7, &regDE.hi);
				   fprintf(output, OPRR, "BIT", "7", "D");
                   break;
        case 0x7b: cycles = Bit(7, &regDE.lo);
				   fprintf(output, OPRR, "BIT", "7", "E");
                   break;
        case 0x7c: cycles = Bit(7, &regHL.hi);
				   fprintf(output, OPRR, "BIT", "7", "H");
                   break;
        case 0x7d: cycles = Bit(7, &regHL.lo);
				   fprintf(output, OPRR, "BIT", "7", "L");
                   break;
        case 0x7e: cycles = Bit(7, &mainMemory[regHL.word]);
				   fprintf(output, OPRR, "BIT", "7", "(HL)");
                   break;
        case 0x7f: cycles = Bit(7, &regAF.hi);
